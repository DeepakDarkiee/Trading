...
def getStockData(stock):
    ...
    for symbol in stock_list:
        ...
        read_file = pandas.read_excel("{}.xlsx".format(symbol)) 
        print(read_file)
        read_file.to_csv ("{}.csv".format(symbol),  index = None, header=True) 
        df = pandas.DataFrame(pandas.read_csv("{}.csv".format(symbol))) 

        

        percentChange = dict(read_file)['Unnamed: 8'][maxRow-2] #wb.sheets['stock data'].range('K{}'.format(maxRow)).value
        totalAvg = dict(read_file)['Unnamed: 10'][maxRow-2] #wb.sheets['stock data'].range('I{}'.format(maxRow)).value
...
def neuralNetPrediction(symbol):
    ...
    try:
        wb = xw.Book('{}.csv'.format(symbol))
    except:
        getStockData([symbol])
        wb = xw.Book('{}.csv'.format(symbol[0]))
...
def main():
    ...
    if action.lower() == 'clean':
        clean()
    if action == '1':
        top_stocks_no=int(input("How many top stocks you want to get \n"))
        getTopStocks(top_stocks_no)
    elif action == '2':
        getStockData('All')
        print(stock_list)
    elif action == '3':
